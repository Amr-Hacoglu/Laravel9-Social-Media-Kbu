@include('admin.Content.indexedit')
    ...
    <div class="col-sm-12">
        <div id="post-modal-data" class="card card-block card-stretch card-height">
            <div class="card-header d-flex justify-content-between">
                ...
                <hr>
                <div class="header-title">
                    <a href="{{route('admin.Content.create')}}" class="btn btn-primary">Add new post</a>
                    <a href="{{route('admin.Content')}}" class="btn btn-secondary">Main page</a>
                </div>
            </div>
            <div class="card-body">
                <form method="post" action="{{route('admin.Content.update',['id'=>$data->id])}}" enctype="multipart/form-data" >
                    @csrf
                    <input type="text" value="{{$data->desc}}" class="form-control rounded" placeholder="Write something here..." name="desc" style="border:none;">
                <hr>
                <ul class=" post-opt-block d-flex list-inline m-0 p-0 flex-wrap">
                    <li class="me-3 mb-md-0 mb-2">

                            <label for="image" >Photo/Video</label>
                            <input value="{{$data->image}}" type="file" class="img-fluid me-2" name="image">
